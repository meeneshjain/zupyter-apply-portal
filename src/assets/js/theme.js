...
(function ($) {
	...
	$(document).on("mouseover", '.primary-menu ul.navbar-nav li.dropdown, .login-signup ul.navbar-nav li.dropdown', function () {
		if ($(window).width() > 991) {
	...
	$(document).each('.primary-menu .dropdown-menu', function () {
	...
	// DropDown Arrow
	$(document).find('.primary-menu').find('a.dropdown-toggle').append($('<i />').addClass('arrow'));
	...
	$(document).each(".owl-carousel", function (index) {
		var a = $(this);
		$(this).owlCarousel({
	...
	$(document).on('hide.bs.modal', '#videoModal', function (e) {
		$(document).find("#video").attr('src', $(document).findvideoSrc);
	})

	/*---------------------------------------------------
		tooltips
	----------------------------------------------------- */
	$(document).find('[data-toggle=\'tooltip\']').tooltip({ container: 'body' });
